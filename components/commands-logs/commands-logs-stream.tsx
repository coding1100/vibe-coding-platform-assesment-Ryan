...
export function CommandLogsStream() {
  ...
  useEffect(() => {
    if (sandboxId) {
      // Fetch logs for commands that don't have logs yet (either running or completed)
      // We check if logs exist by seeing if the command has any logs in the store
      for (const command of commands) {
        // Only fetch logs if we haven't started fetching yet AND the command has a cmdId
        // This includes both running commands (exitCode undefined) and completed commands (exitCode defined)
        if (command.cmdId && !ref.current[command.cmdId] && (!command.logs || command.logs.length === 0)) {
